...
なお、ここで扱うCLAUDE.md設計をそのまま使えるスキルとしてまとめ、関連スキルと一緒に無料リポで配布しています（記事末にリンク）。
...
この3行で「AIが暴走する」問題の多くが解消されるとされる目安です（個人差あり）。
...
## CLAUDE.md設計スキルを無料配布中

この記事で解説したCLAUDE.md設計を、そのまま自分のプロジェクトにコピーして使えるスキルとして無料リポで公開しています。

**無料スターター（GitHub・CC BY 4.0）:**
https://github.com/noguso245-jpg/claude-code-skills-starter

中身は、Claude Code実戦投入の4本のスキル — **CLAUDE.md設計 / 計画ファースト開発（PIV: Plan-Implement-Verify）/ AIコミット戦略 / アジャイルなプロンプト設計** — が日本語・英語の両方で入っています。商用利用も改変も自由です。
