...
📦 この記事のスラッシュコマンドの土台になるCLAUDE.md設計・プロンプト設計スキルは、無料リポ（GitHub・CC BY 4.0）で配布中です。すぐ試したい方は記事末の「今すぐ試す」へ。
...
## スラッシュコマンドの土台スキルを、無料で今すぐ試す

スラッシュコマンドは「プロンプト設計」と「計画ファースト開発（PIV）」を土台にすると安定します。その2本を含む4スキルを無料リポで公開中です（日本語・英語・CC BY 4.0、商用利用可）。
...
**③ 役に立ったら ⭐ Star を押す**
⭐ は「同じく定型プロンプトに消耗している人に、このリポが見つかりやすくなる」応援です。
